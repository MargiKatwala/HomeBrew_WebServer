...
//Margi Katwala
// proj -5
//404 template help https://www.w3schools.com/howto/tryit.asp?filename=tryhow_css_loader
//refrence for opening dir http://pubs.opengroup.org/onlinepubs/009604599/functions/opendir.html
...
#include <sys/uio.h>
#include <unistd.h>
#include <dirent.h> // added new one
#define BACKLOG (10)
...
void serve_request(int client_fd) {
    ...
    requested_file = parseRequest(client_buf); // check what type of file is it ?
    
    //webserver requested html application
    if(strstr(requested_file, ".html"))
    {
        send(client_fd,request_str, strlen(request_str),0);
    }
    //webserver requested pdf application
    ...
    //webserver requested ico application
    ...
    //webserver requested gif application
    ...
    //webserver requested jpg application
    ...
    //webserver requested jpeg application
    if(strstr(requested_file, ".jpeg"))
    {
        char * request_strJPEG = "HTTP/1.0 200 OK\r\n"
        "Content-type: image/jpeg; charset=UTF-8\r\n\r\n";
        send(client_fd,request_strJPEG, strlen(request_strJPEG),0);
        
    }
    //webserver requested PNG application
    ...
    //next three lines given in lab
    filename[0] = '.';
    strncpy(&filename[1], requested_file,4095);
    read_fd = open(filename,0,0); //returns -1 if doesn't exits if not positive num
    ...
    {
        //open dir using the file name
        d = opendir(filename);
        if(d)
        {   //index.html
            strcat(filename, "/index.html");
            match = stat(filename, &index);
            //stat check on webserver
            if(match >= 0)
            {
                send(client_fd,request_str, strlen(request_str),0);
